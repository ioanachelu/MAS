...
class BA():
    ...
    def process_messages(self):
        while self.mq:
            ...
            if message.type == 'partnership':
                print("[Message] BA {} received partnership message from {}".format(self.name, message.partner_of_cell.name))
            ...
            if message.type == 'partnership_cancelation':
                print("[Message] BA {} received partnership cancelation message from {}".format(self.name, message.sender.name))
            ...
            if message.type == 'reservation_cancelation':
                print("[Message] BA {} received reservation cancelation message from {}".format(self.name, message.sender.name))
                if self.reservation:
                    self.cancel_reservation()

            if message.type == 'reservation':
                print("[Message] BA {} received reservation message from {} for cell [{}, {}, {}]".format(self.name,
                                                                                                message.sender.name,
    ...
    def move_to(self, cell):
        print("[Info] BA {} moved from cell DAY {} TIME {} ROOM {} to cell DAY {} TIME {} ROOM {}".format(self.name,
                                                                                                   self.cell.day,
                                                                                                   self.cell.time_slot,
                                                                                                   self.cell.room,
                                                                                                   cell.day,
    ...
    def partner_with(self, baj):
        print("[Info] BA {} partners with {}".format(self.name, baj.name))
        self.cancel_partnership()
    ...
    def reserve_crt_cell(self):
        print("[Info] BA {} reserves cell: DAY {}, TIME {}, ROOM {}".format(self.name, self.cell.day, self.cell.time_slot,
                                                                     self.cell.room))
    ...
    def process_crt_cell(self):
        ...
        if BA.compatible_cell(self, self.cell, self.relax) and \
            (not self.reservation and (BA.rCost(self, self.cell) < BA.rCost(self.cell.reservation, self.cell) or
                                           (BA.rCost(self, self.cell) == BA.rCost(self.cell.reservation, self.cell) and
                                            BA.rlessTime(self, self.cell.reservation)))):
            self.reserve_crt_cell()
            return 1

        if BA.compatible_cell(self, self.cell, self.relax) and \
                (self.reservation and self.cell != self.rCell and self.cell.reservation is not None and
                     (BA.rCost(self, self.cell) < BA.rCost(self.cell.reservation, self.cell) or
                          (BA.rCost(self, self.cell) == BA.rCost(self.cell.reservation, self.cell) and
                               BA.rlessTime(self, self.cell.reservation))) and
                     (BA.rCost(self, self.cell) < BA.rCost(self, self.rCell))):
            self.reserve_crt_cell()
            return 1
    ...
    def process_encountered_bas(self):
        ...
        for baj in self.knows:
            ...
            # or I already achieved the partnership goal and my partner is better than the new opportunity in front of me
            # ignore this guy
            ...
            # if I don;t have a partner and he doesn't have a partner
            ...
            if not self.partner and baj.partner and (BA.pCost(self, baj) < BA.pCost(baj, baj.partnership) or BA.deadlock(self)):
                self.partner_with(baj)
                return 1
        return 0
        ...
        if self.partner and self.reservation:
            ...
            if BA.rCost(self, self.rCell) == 0:
                ...
                if self.cell == self.rCell:
                    print("[Info] Reservation is optimal and i am already in that cell")
                else:
                    print("[Info] Reservation is optimal and i am going there cell")
                    self.move_to(self.rCell)
                self.add_bas_to_memory()
            else:
                print("[Info] Reservation is not optimal. Exploring other opportunities")
                self.explore()
                self.add_bas_to_memory()
                rez = self.process_crt_cell()
                if rez == 0:
                    print("[Info] Cell doesn't fit with constraints")
                if rez == 2:
            ...
            if rez == 0:
                print("[Info] No BAs fit with constraints")
    ...
    def nonCompatible_constraints(c, cc, relax=False):
        if (cc.type == "U" or c.type == "U") and not relax:
            return True
        if not relax and (((c.type == "T" or c.type == "SG")
                           and cc.type == "A" and 'projector' not in cc.tools) or
                              ((cc.type == 'T' or cc.type == 'SG')
                               and c.type == 'A' and 'projector' not in c.tools)):
            return True
